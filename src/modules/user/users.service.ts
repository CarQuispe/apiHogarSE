...
export class UsersService {
  ...
  async create(
    ...
    try {
      ...
      await transporter.sendMail({
        ...
        html: `<div>Hola ${nombre},</div>
               <div>Tu usuario ha sido creado exitosamente.</div>
               <div>Tu CI de acceso es: <b>${ci}</b></div>
  ...
  async findByCi(ci: string): Promise<User | null> {
    return await this.usersRepository.findOne({ where: { ci } });
  }
  ...
  async validateUser(ci: string, password: string): Promise<User | null> {
    const user = await this.findByCi(ci);
  ...
  async updateUser(id: number, updateData: Partial<User>) {
    ...
    const { password, ci, email, ...safeData } = updateData;
